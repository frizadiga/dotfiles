...
local function handle_screen_change()
  ...
  --   "/bin/bash",
  --   handleOutput,
  --   -- {os.getenv('HOME') .. '/workspace/main/tools/audio-switch-by-display.sh'}
  -- )
  -- task:start()

  -- os.execute('zsh ' .. os.getenv('HOME') .. '/workspace/main/tools/audio-switch-by-display.sh')

  local output, status, type, rc = hs.execute(
    'zsh ' .. os.getenv('HOME') .. '/workspace/main/tools/audio-switch-by-display.sh',
    true
  )
  ...
  -- hs.execute('bash ' .. os.getenv('HOME') .. '/workspace/main/tools/layout-corrector/layout-corrector.sh', true)
end
